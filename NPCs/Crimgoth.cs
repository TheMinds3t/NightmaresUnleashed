...
{
	public class Crimgoth : ModNPC
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Crimgoth");
		}
		...
		public override bool CheckDead()
		{
			if(npc.life <= 0)
			{
				for(int i = 0; i < Main.rand.Next(6) + 5; ++i)
				{
					...
					Main.npc[danger].localAI[0] = 1.0f;
				}
			}
		 ...
		 public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            ...
            return Main.player[Main.myPlayer].ZoneCrimson && Main.hardMode ? ret * TrueEater.GetSpawnModifier(npc,spawnInfo) : 0f;
        }
		...
		public override void NPCLoot()
		{
			...
			{
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("CrimGothEye"), 1 + Main.rand.Next(2));
			}
		}
	}
}